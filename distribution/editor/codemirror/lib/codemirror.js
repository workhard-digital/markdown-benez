(function(mod) {
  ...
  function applyTextInput(cm, inserted, deleted, sel) {
    ...
    if (cm.state.pasteIncoming && sel.ranges.length > 1) {
      if (lastCopied && lastCopied.join("\n") == inserted) multiPaste = sel.ranges.length % lastCopied.length == 0 && map(lastCopied, splitLines); else if (textLines.length == sel.ranges.length) multiPaste = map(textLines, function(l) {
        return [ l ];
      });
    }
    for (var i = sel.ranges.length - 1; i >= 0; i--) {
      ...
      if (range.empty()) {
        if (deleted && deleted > 0) from = Pos(from.line, from.ch - deleted); else if (cm.state.overwrite && !cm.state.pasteIncoming) to = Pos(to.line, Math.min(getLine(doc, to.line).text.length, to.ch + lst(textLines).length));
      }
      ...
      function rehide() {
        ...
        if (te.selectionStart != null) {
          ...
          var i = 0, poll = function() {
            if (display.selForContextMenu == cm.doc.sel && te.selectionStart == 0 && input.prevInput == "​") operation(cm, commands.selectAll)(cm); else if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500); else display.input.reset();
          };
  ...
  ContentEditableInput.prototype = copyObj({
    init: function(display) {
      ...
      on(div, "input", function() {
        ...
        if (!input.pollContent()) runInOp(input.cm, function() {
          regChange(cm);
        });
      });

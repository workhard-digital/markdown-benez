/*! For license information please see markdown-it.js.LICENSE.txt */
(function(f) {
  ...
  return function e(t, n, r) {
    ...
    17: [ function(require, module, exports) {
      ...
      default_rules.fence = function(tokens, idx, options, env, self) {
        var token = tokens[idx], langName = "", highlighted;
        if (token.info) {
    ...
    24: [ function(require, module, exports) {
      ...
      module.exports = function html_block(state, startLine, endLine, silent) {
        var ch, match, nextLine, token, pos = state.bMarks[startLine], max = state.eMarks[startLine], shift = state.tShift[startLine];
        pos += shift;
    ...
    36: [ function(require, module, exports) {
      ...
      function process_inlines(tokens, state) {
        var i, token, text, t, pos, max, thisLevel, item, lastChar, nextChar, isLastPunctChar, isNextPunctChar, isLastWhiteSpace, isNextWhiteSpace, canOpen, canClose, j, isSingle, stack;
        stack = [];
    ...
    54: [ function(require, module, exports) {
      ...
      var src_path = exports.src_path = "(?:" + "[/?#]" + "(?:" + "(?!" + src_ZCc + "|[()[\\]{}.,\"'?!\\-]).|" + "\\[(?:(?!" + src_ZCc + "|\\]).)*\\]|" + "\\((?:(?!" + src_ZCc + "|[)]).)*\\)|" + "\\{(?:(?!" + src_ZCc + "|[}]).)*\\}|" + '\\"(?:(?!' + src_ZCc + '|["]).)+\\"|' + "\\'(?:(?!" + src_ZCc + "|[']).)+\\'|" + "\\'(?=" + src_pseudo_letter + ").|" + "\\.{2,3}[a-zA-Z0-9%]|" + "\\.(?!" + src_ZCc + "|[.]).|" + "\\-(?!" + src_ZCc + "|--(?:[^-]|$))(?:[-]+|.)|" + "\\,(?!" + src_ZCc + ").|" + "\\!(?!" + src_ZCc + "|[!]).|" + "\\?(?!" + src_ZCc + "|[?])." + ")+" + "|\\/" + ")?";
